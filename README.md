# Building Agent Systems in web3/web2 

Exploring the intersection of AI agents and Web3 building autonomous systems, smart contracts, and intelligent applications.

---

## Hackathon Wins (19x)

- Aug 28 — Nemo3 — Civic Auth Hackathon — First Prize  
- Sep 7 — Growth — Civic Auth (Second Round) — Winner  
- Sep 21 — Code and Cargo — Filecoin Hackathon — Filecoin Bounty  
- Nov 13 — Sol Terminal — Pharos — 3rd Prize  
- Nov 19 — Proof on Content — Blog on AI & Web3 — 6th Place  
- Dec 8 — Curious Quench — Somnia SDS — Top Project  
- Dec 14 — Afro-Pop Ticket — VIBE CODE — 1st Place  
- Jan 5 — Grok Prompthathon — Creative Winner
- BOS Hackathon — 3rd Place
- https://devpost.com/software/surgicalai  
- https://devpost.com/software/genie-1muk2z  
- https://devpost.com/software/braindump-nek3cg
- anything hackathon(nonchlant winner) - 2026
- zo computer top bonus prize winner - 2025
- cline hackathon review winner- 2025
- arc nanopayment hachathon 1st place for featherlessAi track- 2026
- anything hackathon bonus prize - 2025
- Arc usdc x402 winner 2026- project ShadowNPM
- jazz hacks  - https://devpost.com/software/killbill-wam45d
---

## Blog Posts

- [The Ralf Wiggum Breakdown](https://dev.to/ibrahimpima/the-ralf-wiggum-breakdown-3mko)  
- [The Shortest SEO Course for Tech Bros](https://dev.to/ibrahimpima/the-shortest-seo-course-for-tech-bros-4klo)  
- [Smart Contracts for Dummies](https://dev.to/ibrahimpima/smart-contracts-for-dummies-5dok)  
- [Shadcn Button Component from Scratch](https://dev.to/ibrahimpima/shadcn-button-component-from-scratch-4aj6)  

---
